...
# rating_matrix=np.load(input_path+'normed_rating_matrix_100_user_500_bookmark.npy')
# rating_matrix_mask=np.load(input_path+'rating_matrix_mask_100_user_500_bookmark.npy')
...
plt.legend(loc=1, fontsize=10)
plt.tight_layout()
plt.savefig(path+'beta_delicious_user_num_%s_item_num_%s'%(user_num, item_num)+'.png', dpi=300)
plt.savefig(path+'beta_delicious_user_num_%s_item_num_%s'%(user_num, item_num)+'.eps', dpi=300)
plt.show()