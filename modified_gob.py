...
class Modified_GOB():
	...
	def update_beta(self):
		self.beta=self.sigma*np.sqrt(2*np.log(np.linalg.det(self.covariance)**(1/2)*np.linalg.det(self.alpha*self.A)**(-1/2)/self.delta))+self.alpha*np.linalg.norm(self.true_user_feature_vector)
	...
	def update_user_feature(self, true_payoff, selected_item_feature, user_index):
		L_x=self.L.copy()
		I_d=np.identity(self.dimension)
		I_m=np.identity(self.user_num)
		A_x=np.kron(L_x+I_m, I_d)
		A_x[A_x<1e-4]=0
		A_inv=np.sqrt(np.linalg.pinv(A_x))
		x_share=np.dot(A_inv, selected_item_feature)
		#x_share[user_index*self.dimension:(user_index+1)*self.dimension]=selected_item_feature[user_index*self.dimension:(user_index+1)*self.dimension]
	...
	def run(self,  user_array, item_pool_array, iteration):
		...
		for time in range(iteration):	
			print('time/iteration', time, iteration, '~~~Modified GOB')
