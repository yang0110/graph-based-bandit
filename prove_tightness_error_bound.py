...
for ind, thres in enumerate(thres_list):
	...
	error_bound_graph=np.zeros((user_num, iteration))
	error_bound_ridge=np.zeros((user_num, iteration))

	user_error_bound_ridge=np.zeros((user_num, iteration))
	user_error_bound_graph=np.zeros((user_num, iteration))

	confidence_graph=np.zeros((user_num, iteration))
	confidence_ridge=np.zeros((user_num, iteration))
	confidence_ridge_old=np.zeros((user_num, iteration))
	...
	confidence_bound_graph=np.zeros((user_num, iteration))
	confidence_bound_ridge=np.zeros((user_num, iteration))
	confidence_bound_ridge_old=np.zeros((user_num, iteration))
	...
	for i in range(iteration):
		...
		error_bound_ridge[user_index, i]=ridge_error_bound(user_v[user_index], user_f_matrix_ridge[user_index], alpha, user_xnoise[user_index])
		error_bound_graph[user_index, i]=graph_error_bound(user_index, user_v[user_index], normed_lap, user_f_matrix_ls, user_f_matrix_graph[user_index], alpha, user_xnoise[user_index])
		...
		confidence_ridge[user_index, i]=ridge_true_UCB(user_v[user_index], user_f_matrix_ridge[user_index], alpha, user_xnoise[user_index])
		confidence_graph[user_index, i]=graph_true_UCB(user_index, user_v[user_index], normed_lap, user_f_matrix_ls, user_f_matrix_graph[user_index], alpha, user_xnoise[user_index])
		...
		confidence_bound_ridge[user_index, i]=ridge_UCB(user_v[user_index], user_f_matrix_ridge[user_index], alpha, user_xnoise[user_index])
		confidence_bound_ridge_old[user_index, i]=ridge_UCB_old(user_v[user_index], user_f_matrix_ridge[user_index], alpha, user_xnoise[user_index])
		confidence_bound_graph[user_index, i]=graph_UCB(user_index, user_v[user_index], normed_lap, user_f_matrix_ls, user_f_matrix_graph[user_index], alpha, user_xnoise[user_index])
	...
	mean_error_ridge=np.mean(user_error_ridge, axis=0)
	mean_error_graph=np.mean(user_error_graph, axis=0)

	mean_error_bound_ridge=np.mean(user_error_bound_ridge, axis=0)
	mean_error_bound_graph=np.mean(user_error_bound_graph, axis=0)
	...
	plt.figure(figsize=(5,5))
	plt.plot(user_error_ridge[0][user_error_ridge[0]>0], label='Error Ridge')
	plt.plot(user_error_graph[0][user_error_graph[0]>0], label='Error Graph')
	plt.plot(user_error_bound_ridge[0][user_error_bound_ridge[0]>0], label='Error-Bound Ridge')
	plt.plot(user_error_bound_graph[0][user_error_bound_graph[0]>0], label='Error-Bound Graph')
	plt.ylabel('Estimation Error Bound', fontsize=12)
	plt.xlabel('Time', fontsize=12)
	plt.tight_layout()
	plt.legend(loc=0, fontsize=10)
	plt.savefig(path+'error_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.png', dpi=300)
	plt.savefig(path+'error_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.eps', dpi=300)
	plt.show()

	plt.figure(figsize=(5,5))
	#plt.plot(user_confidence_bound_ridge[0][user_confidence_bound_ridge[0]>0], '*', label='True UCB Ridge')
	#plt.plot(user_confidence_bound_graph[0][user_confidence_bound_graph[0]>0], '*', label='True UCB Graph')
	plt.plot(user_confidence_bound_ridge_old[0][user_confidence_bound_ridge_old[0]>0], label='UCB Ridge (Old)')
	plt.plot(user_confidence_bound_ridge[0][user_confidence_bound_ridge[0]>0], label='UCB Ridge (New)')
	plt.plot(user_confidence_bound_graph[0][user_confidence_bound_graph[0]>0], '-.', label='UCB Graph')
	plt.ylabel('Confidence Upper Bound (UCB)', fontsize=12)
	plt.xlabel('Time', fontsize=12)
	plt.tight_layout()
	plt.legend(loc=0, fontsize=10)
	plt.savefig(path+'confidence_upper_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.png', dpi=300)
	plt.savefig(path+'confidence_upper_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.eps', dpi=300)
	plt.show()
	...
	plt.axis('off')
	plt.savefig(path+'graph_of_error_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.png', dpi=300)
	plt.savefig(path+'graph_of_error_bound_tighness_node_num_edge_num_thres_%s_%s_%s'%(user_num, edge_num, int(thres*10))+'.eps', dpi=300)
	plt.show()
