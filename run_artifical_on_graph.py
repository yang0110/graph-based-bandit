...
ws_adj=WS_graph(user_num, np.int(np.log2(user_num)), 0.2)
...
plt.axis('off')
plt.savefig(path+'network_ws'+'.eps', dpi=300)
plt.clf()
...
for lambda_ in lambda_list:
	...
	# plt.figure(figsize=(5,5))
	# plt.plot(linucb_error,'-.', markevery=0.1, label='Ridge')
	# #plt.plot(ts_error,'-p',  markevery=0.1, label='TS')
	# plt.plot(gob_error, '-o', color='orange', markevery=0.1, label='GOB')
	# plt.plot(lapucb_sim_error, '-s', markevery=0.1, label='Approx')
