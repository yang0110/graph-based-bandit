...
alpha2_length=5
alpha2_list=np.linspace(0, 0.25, alpha2_length)

linucb_regret_matrix_alpha=np.zeros((alpha2_length, iteration))
linucb_error_matrix_alpha=np.zeros((alpha2_length, iteration))
club_regret_matrix_alpha=np.zeros((alpha2_length, iteration))
club_error_matrix_alpha=np.zeros((alpha2_length, iteration))
...
for index, alpha_2 in enumerate(alpha2_list):
	...
	for l in range(loop):
		...
		club_model = CLUB(dimension, user_num, item_num, pool_size, item_feature_matrix, user_feature_matrix, true_payoffs, 0.25, alpha_2, delta, sigma, beta, state)
	...
	club_regret_matrix_alpha[index]=club_regret
	club_error_matrix_alpha[index]=club_error

	plt.figure(figsize=(5,5))
	plt.plot(linucb_regret,'-.', markevery=0.1, label='LinUCB')
	plt.plot(club_regret, '-o', color='r', markevery=0.1, label='CLUB')
	plt.ylabel('Cumulative Regret', fontsize=16)
	plt.xlabel('Time', fontsize=16)
	plt.legend(loc=2, fontsize=16)
	plt.title('Alpha2=%s'%(alpha_2))
	plt.tight_layout()
...
plt.figure(figsize=(5,5))
p=plt.plot(club_regret_matrix_alpha.T)
plt.legend(p, alpha2_list)
plt.xlabel('Time')
plt.ylabel('Cumulative Regret')
plt.title('CLUB')
