...
for ind, sm in enumerate(smooth_list):
	plt.plot(lapucb_regret_matrix[ind], '-', markevery=0.1, label='sm=%s'%(np.round(smoothness_list[ind], decimals=2)))
plt.ylim([0,80])
plt.ylabel('Cumulative Regret', fontsize=16)
plt.xlabel('Time', fontsize=16)
plt.legend(loc=0, fontsize=14)
plt.tight_layout()
plt.savefig(path+'graphucb_vs_smooth'+'.png', dpi=100)
plt.show()

plt.figure(figsize=(5,5))
for ind, sm in enumerate(smooth_list):
	plt.plot(lapucb_sim_regret_matrix[ind], '-*', markevery=0.1, label='sm=%s'%(np.round(smoothness_list[ind], decimals=2)))
plt.ylim([0,80])
plt.ylabel('Cumulative Regret', fontsize=16)
plt.xlabel('Time', fontsize=16)
plt.legend(loc=0, fontsize=14)
plt.tight_layout()
plt.savefig(path+'graphucb_local_vs_smooth'+'.png', dpi=100)
plt.show()
