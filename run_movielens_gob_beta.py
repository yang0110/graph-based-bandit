...
beta_length=5
beta_list=np.linspace(0,0.25, beta_length)
...
for index, b in enumerate(beta_list):
	...
	for l in range(loop):
		...
		gob_model=GOB(dimension, user_num, item_num, pool_size, item_feature_matrix, user_feature_matrix, true_payoffs, true_adj, true_lap, 0.25, delta, sigma, b, state)
	...
	gob_error_matrix_beta[index]=gob_error
...
plt.figure(figsize=(5,5))
p=plt.plot(linucb_regret_matrix_beta.T)
plt.legend(p, beta_list)
plt.xlabel('Time')
...
plt.figure(figsize=(5,5))
p=plt.plot(gob_regret_matrix_beta.T)
plt.legend(p, beta_list)
plt.xlabel('Time')
plt.ylabel('Cumulative Regret')
plt.title('Gob.Lin')
