...
class Adaptive_COVUCB():
	...
	def update_A(self):
		...
		self.A=-(1/(2*self.beta))*np.outer(self.user_feature, self.user_feature)
		self.covariance+=self.alpha*(self.A-A_t_1)

	def run(self,  user_array, item_pool_array, iteration, true_cov):
		cumulative_regret=[0]
		learning_error_list=np.zeros(iteration)
		cov_error=np.zeros(iteration)
		for time in range(iteration):	
			...
			cov_error[time]=np.linalg.norm(self.A-np.kron(true_cov, np.identity(self.dimension)))

		return cumulative_regret, learning_error_list, cov_error