...
class LINUCB_SIN(): # together update feature and confidence bound
	def __init__(self, dimension, user_num, item_num, pool_size, item_feature_matrix, true_user_feature_matrix, true_payoffs, alpha, delta, sigma):
		...
		self.I=np.kron(np.identity(self.user_num), np.identity(self.dimension))
	...
	def run(self,  user_array, item_pool_array, iteration):
		...
		for time in range(iteration):	
			print('time/iteration', time, iteration,'~~~LINUCB SIN')
		...
		return cumulative_regret, learning_error_list, np.array(self.beta_list), np.array(self.x_norm_list), np.array(self.true_confidence_bound)
